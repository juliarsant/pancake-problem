...
Implementation of an Uniform-Cost Search Algorithm to solve the pancake problem.
"""
...
class Node():
    ...
    def __eq__(self, other):
        return (self.backward == other.backward and self.stack == other.stack)
...
class PriorityQueue():
    ...
    def compare(self, child_tuple):
        for i in range(len(self.queue)):
            if self.queue[i].stack == child_tuple.stack and self.queue[i].backward > child_tuple.backward:
                del self.queue[i]
                return True
            

"""
Implementation of the UCS Algorithm. It uses the backwards cost (Number of Flips)
to make a decision on the next node. 
"""
def ucs(pancakes) -> bool:
    ...
    while True:
        ...
        if root.goal_state():
            print(f"Flips Taken: {root.backward}")
            return True
